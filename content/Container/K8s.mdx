...
对于一些高流量的前端应用，Kubernetes 可以根据流量自动扩展容器的副本数。Kubernetes 的 Horizontal Pod Autoscaler（HPA）根据 CPU 或内存利用率自动调整应用实例的数量，确保前端应用在高峰期能够承载更多的用户请求。

例如，当一个电商网站的流量暴增时，Kubernetes 会自动增加前端应用的实例，以确保用户请求得到快速响应。
